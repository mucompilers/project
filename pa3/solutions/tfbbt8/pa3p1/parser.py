...
def p_crate(p):
    ...
    #printAST(AST("crate", AST("items", p[1])), 0)
...
def addToStructList(whichList, whichType, node, arguments, returntype):
    ...
    for item in whichList:
        if item.value == node:
            outputFile.write("Error: duplicate definition of " + whichType + " or enum `" + node + "`.\n")
            sys.exit(-1)
...
#         $$ = build_AST("struct", l);}'''
    newList = []
    newList.append(p[1])
    newList.append(newList, AST("field-inits", p[3]))
...
def printAST(node, depth):
    if node:
        ...
        if hasattr(node, 'next'):
            printNext(node, depth)        
    
def printChildren(node, depth):
    for children in node:
        if children:
            printAST(children, depth + 1)

def printNext(node, depth):    
    for next in node:
        if next:
            printAST(next, depth + 1)
