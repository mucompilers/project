...
static void insert_new_env_fn(struct pair* pair, gpointer paramArr[]);
static void insert_new_env_enum(struct pair* pair, gpointer paramArr[]);
static void insert_new_env_struct(struct pair* pair, gpointer paramArr[]);
...
struct item* item_fn_def(Symbol id, GList* params, struct type* ret, struct exp* block) {
      ...
      //add fn def to env
	  if (env_contains(environment, id)) {
	  	printf("Error: duplicate definition of function `%s`.\n", symbol_to_str(id));
	  	is_error = 1;
	  } else {
	  	//check if it's main and set flag
	  	if (strncmp(symbol_to_str(id), "main", 4) == 0) {
	  		is_main = 1;
	  		if (params != NULL) {
	  	...
	  	env_insert_def(environment, id, n);
	  }
	  
	  //array so we can pass multiple values to foreach
	  gpointer paramArr[2];
	  paramArr[0] = n->env;
	  paramArr[1] = &id;
	  g_list_foreach(params, (GFunc)insert_new_env_fn, (gpointer) paramArr);
...
struct item* item_enum_def(Symbol id, GList* ctors) {
	  ...
	  paramArr[0] = n->env;
	  paramArr[1] = &id;
	  g_list_foreach(ctors, (GFunc)insert_new_env_enum, (gpointer) paramArr);
...
struct item* item_struct_def(Symbol id, GList* fields) {
	  ...
	  paramArr[0] = n->env;
	  paramArr[1] = &id;
	  g_list_foreach(fields, (GFunc)insert_new_env_struct, (gpointer) paramArr);
...
static void insert_new_env_fn(struct pair* pair, gpointer paramArr[]) {
    ...
    struct env* env = paramArr[0];
    Symbol* parent_name = paramArr[1];
	if(env_contains(env, pair->param.pat->bind.id)) {
		printf("Error: identifier `%s` is bound more than once in the parameter list for the function `%s`.\n" , symbol_to_str(pair->param.pat->bind.id), symbol_to_str(*parent_name));
		is_error = 1;
	} else {
		env_insert(env, pair->param.pat->bind.id, pair->param.type);
	}
...
static void insert_new_env_enum(struct pair* pair, gpointer paramArr[]) {
    ...
    struct env* env = paramArr[0];
    Symbol* parent_name = paramArr[1];
    struct type* placeholder = type_unit();
	if(env_contains(env, pair->ctor_def.id)) {
		printf("Error: duplicate definition of the constructor `%s` for the enum `%s`.\n" , symbol_to_str(pair->ctor_def.id), symbol_to_str(*parent_name));
		is_error = 1;
	} else {
		env_insert(env, pair->ctor_def.id, placeholder);
	}
}
...
static void insert_new_env_struct(struct pair* pair, gpointer paramArr[]) {
    ...
    struct env* env = paramArr[0];
    Symbol* parent_name = paramArr[1];
	if(env_contains(env, pair->field_def.id)) {
		printf("Error: duplicate declaration of the field `%s` of the struct `%s`.\n" , symbol_to_str(pair->field_def.id), symbol_to_str(*parent_name));
		is_error = 1;
	} else {
		env_insert(env, pair->field_def.id, pair->field_def.type);
	}
}