...
struct env* env_new(void) {
      struct env* new = malloc(sizeof(*new));
      new->types = g_hash_table_new(NULL, NULL);
      new->vars = g_hash_table_new(NULL, NULL);
      return new;
}
...
void env_insert(struct env* env, Symbol symbol, struct type* type) {
      assert(env);
      assert(symbol.kind == SYMBOL_TYPE || symbol.kind == SYMBOL_VAR || symbol.kind == SYMBOL_CTOR || symbol.kind == SYMBOL_FIELD);
