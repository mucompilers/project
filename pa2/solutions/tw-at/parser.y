...
%type<gast>  item fn-def fn-params fn-param pat-type-annot ret enum-def enum-ctor-defs
	     ...
	     addr-of-mut assign assign-op fn-call exps if while loop box-new arr-index flup
	     flup1 flup2 match match-arms match-arm pat-or pat name-bind pat-deref pat-arr pat-enum
...
/*BEGIN TYPES*/
/*%type<num> or and eq neq lt gt geq leq add sub mul div rem neg not deref addr-of*/  

%left COMMA
%right ASSOPS		/*PRECEDENCE TOKEN*/
%left BOOL_OR 
%left BOOL_AND
...
addr-of-mut:	  ADDRESS MUT exp  %prec ADDRMUT {
			...
			/*ASSIGNMENT : T's**/
assign:	ID assign-op exp %prec ASSOPS{
			g_node_prepend($2, $3);
			g_node_prepend($2, ast_node_new(ID, $1));
			$$ = $2;		
			}
|	flup assign-op exp %prec ASSOPS	{
			g_node_prepend($2, $3);
			g_node_prepend($2, $1);
			$$ = $2;	
			}	
|	arr-index assign-op exp	%prec ASSOPS {
			g_node_prepend($2, $3);
			g_node_prepend($2, $1);
			$$ = $2;
			}
assign-op:	PLUS_ASSIGN				{$$ = ast_node_new(PLUS_ASSIGN, NULL);}
|		SUB_ASSIGN				{$$ = ast_node_new(SUB_ASSIGN, NULL);}
|		MUL_ASSIGN				{$$ = ast_node_new(MUL_ASSIGN, NULL);}
|		DIV_ASSIGN				{$$ = ast_node_new(DIV_ASSIGN, NULL);}
|		REM_ASSIGN				{$$ = ast_node_new(REM_ASSIGN, NULL);}
|		ASSIGNMENT				{$$ = ast_node_new(ASSIGNMENT, NULL);}
...
arr-index: 	ID LSQUARE  exp  RSQUARE {
			GNode * p = ast_node_new(ARRIDX_EXP, NULL);
			g_node_prepend(p, $3);
			g_node_prepend(p, ast_node_new(ID, $1));
			$$ = p;		
			}
|		fn-call LSQUARE  exp  RSQUARE {
			GNode * p = ast_node_new(ARRIDX_EXP, NULL);
			g_node_prepend(p, $3);
			g_node_prepend(p, $1);
			$$ = p;		
			}
| 		arr-index LSQUARE  exp  RSQUARE {
			GNode * p = ast_node_new(ARRIDX_EXP, NULL);
			g_node_prepend(p, $3);
			g_node_prepend(p, $1);
			$$ = p;		
...
|			LITDEC				{$$ = ast_node_new(LITDEC, NULL);}
